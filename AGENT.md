...
- 一次只問一件事，字大、按鈕大，主要動作為整頁寬的大按鈕
- 分類以整列大卡點選（圖示＋主標＋說明），不必打字
...
- 比賽用的舊版精靈（步驟圓點列、pill 分類、多顆小按鈕）以註解保留在 `temple_oracle_v17.html` 中，標示為「比賽用版本」，需要時可整段切回
...
- 最多 300 字
- 可以留白：使用者不想填時，前端依所選分類自動組出請示語句（例如「想請示工作錢財方面目前的運勢，請神明指點該把握與留意的地方」）再送出
- 實際送往後端的 question 仍需滿足最少 2 字，此規則由前端的 `resolveQuestion()` 統一保證，不得在畫面上以「請先輸入問題」阻擋使用者
