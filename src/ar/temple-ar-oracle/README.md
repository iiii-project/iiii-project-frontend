...
- 搖籤：桌機「握拳/握籤」或「雙手上下搖晃」任一種都觸發（門檻已放寬，見 config.js）/ 手機 devicemotion 搖晃偵測 / 純點擊三種輸入模式；
  畫面下方顯示 hands-shake.webp，籤筒放在兩手之間（位置由 styles.css 的 `#draw-stage` CSS 變數調整），搖晃時手與籤筒一起上下抖動
- 捏取抽籤：搖出命中籤後自動出現 hands-pinch.webp（單手，指尖對準命中籤枝頂端），手往上滑動（500ms 內上移超過畫面高度 15%）即抽出
- 擲筊：畫面下方顯示 hands-cup.webp（捧著），3D 筊杯閒置時放在手心凹處（`#bwa-hands` 的 `--cup-x/--cup-y`）；
  擲出時切成 hands-toss.webp（拋擲）並開始筊杯動畫，動畫結束後淡出再接結果判定；非聖筊需重擲時手部圖回到「捧著」
- 主要使用者判定（engine/primary-user.js）：多人入鏡時只認占畫面最大、最靠近中央的那位，其他人的手一律忽略
- 擲筊：單手握拳抓杯/往下丟擲手勢偵測（含速度、加速度輔助判定）、Three.js 3D筊杯渲染與物理動畫、
  聖筊/笑筊/陰筊判定與後續流程（連續聖筊要求、非聖筊重新抽籤）
- 香灰粒子特效、木質敲擊音效（WebAudio即時合成）、墨染金線過場動畫
- 神明實景疊加（`#ritual-overlay`）：插香/抽籤/擲筊三階段進場時，廟宇背景+
  玉皇大帝神像先100%不透明蓋住整個畫面（各階段停留秒數見 flow-controller.js 的
  `RITUAL_OVERLAY_VEIL_MS`），之後淡化到60%。求籤過程中不顯示人物：攝影機只拿來做
  MediaPipe Hands 手勢辨識，畫面不繪製鏡頭影像，也不做人像去背（已移除 SelfieSegmentation）
