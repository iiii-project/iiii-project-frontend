...
import AccountAccessPanel from '@/mobile/components/common/AccountAccessPanel.vue'
      ...
      <template v-else>
        ...
        <section v-else-if="records.length" class="history-layout">
          <div class="record-list"><button v-for="record in records" :key="record.session_id" type="button" :class="{ selected: selected?.session_id === record.session_id }" @click="selected = record"><span>{{ formatDate(record.created_at) }}</span><strong>{{ record.fortune?.title || '尚未完成的求籤' }}</strong><p>{{ record.question }}</p></button></div>
           <article v-if="selected" class="record-detail"><div class="detail-top"><div><p class="eyebrow">{{ selected.category }}</p><h2>{{ selected.fortune?.title || '求籤進行中' }}</h2></div><button type="button" class="delete" :disabled="deletingId === selected.session_id" @click="removeItem(selected.session_id)">刪除</button></div><div class="question"><span>你的問題</span><p>{{ selected.question }}</p></div><div v-if="selected.fortune" class="fortune"><p>{{ selected.fortune.ganzhi }}</p><pre>{{ selected.fortune.poem }}</pre><p>{{ selected.fortune.translation }}</p></div><div v-if="selected.interpretation?.overall_meaning" class="interpretation"><span>解籤紀錄</span><p>{{ selected.interpretation.overall_meaning }}</p></div></article>
        </section>
        <section v-else class="empty-state"><h2>還沒有帳號求籤紀錄</h2><p>登入後從「開始求籤」進入的紀錄會顯示在這裡。</p><RouterLink class="primary" to="/oracle">開始求籤</RouterLink></section>
      </template>
    </main>
  </div>
</template>
