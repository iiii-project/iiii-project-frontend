...
/* 開門聲有兩個來源，改這個常數就能切換：
   true  = 用下載的音檔 door-open.wav（金屬櫃門，站方標示僅供學習參考、禁商用）
   false = 用下方合成的木門聲（自製，無授權疑慮） */
const USE_DOOR_FILE = false
const doorSoundUrl = new URL('../assets/audio/door-open.wav', import.meta.url).href
let doorSound: HTMLAudioElement | null = null
let doorCtx: AudioContext | null = null

function primeDoorSound() {
  if (doorSound) return
  doorSound = new Audio(doorSoundUrl)
  doorSound.preload = 'auto'
  doorSound.volume = 0.85
  doorSound.load()
}

/* ── 開門聲：合成的木門。抓現成音檔授權不明，這裡自己做，也好對準動畫。
   結構：門栓鬆開的木質 clack → 轉軸吱呀（窄帶共振 + stick-slip 顆粒）
        → 門扇轉動的低頻滾動 → 尾端輕輕定位的一聲。 ── */
...
function renderDoorSound(ctx: BaseAudioContext, output: AudioNode, now: number) {
  ...
  // ② 轉軸吱呀：窄帶共振自 280Hz 滑到 760Hz，再用鋸齒 LFO 做 stick-slip 的顆粒感
  ...
  const creakBand2 = ctx.createBiquadFilter() // 第二共振峰，聲音才不會像純哨音
  ...
  const slip = ctx.createOscillator() // 木頭與木頭之間一頓一頓地滑
  ...
  const creakMakeup = ctx.createGain() // 窄帶濾完只剩一點點能量，補回來
...
function openDoor() {
  ...
  openTimer = window.setTimeout(() => (isOpen.value = true), 1500)
}
...
onMounted(() => {
  ...
  primeSound()
  if (USE_DOOR_FILE) primeDoorSound()
...
<template>
  <div class="mobile-home" :class="{ opening: isOpening, opened: isOpen, ascending: isAscending }">
    ...
    <div v-if="!isOpen" class="gate" :class="{ go: isOpening }">
      ...
      <p class="gate-hint">{{ isOpening ? '　' : '輕 觸 推 門' }}</p>
      <button class="gate-hit" type="button" aria-label="推門進入" @click="openDoor"></button>
    </div>
